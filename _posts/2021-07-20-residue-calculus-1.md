...
since $k\sgn(k)$$=|k|$ and the $\sgn(k)$ factor takes into account the cases where $k>0$, and $k<0$ (i.e. the resultant negative sign flips back the terminals so they always go from $-\infty$ to $\infty$ for both cases of $k$). yo
